...
def test_left_join(constructor: Constructor) -> None:
    ...
    result_on_list = df_left.join(df_right, on=["antananarivo", "idx"], how="left")  # type: ignore[arg-type]
...
def test_left_join_overlapping_column(constructor: Constructor) -> None:
    ...
    assert_equal_data(result, expected)
    result = df_left.join(df_right, left_on="antananarivo", right_on="d", how="left")  # type: ignore[arg-type]
    result = result.sort("idx")
    result = result.drop("idx_right")
    expected = {
        "antananarivo": [1, 2, 3],
