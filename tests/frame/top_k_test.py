...
def test_top_k(nw_frame_constructor: Constructor) -> None:
...
def test_top_k_by_multiple(nw_frame_constructor: Constructor) -> None:
    if "polars" in str(nw_frame_constructor) and POLARS_VERSION < (0, 20, 22):
        # bug in old version
        pytest.skip()
    if "duckdb" in str(nw_frame_constructor) and DUCKDB_VERSION < (1, 3):
        pytest.skip()
