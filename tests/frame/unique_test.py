...
def test_unique_invalid(nw_frame_constructor: Constructor) -> None:
    if "duckdb" in str(nw_frame_constructor) and DUCKDB_VERSION < (1, 3):
        pytest.skip()
    df_raw = nw_frame_constructor(data)
    df = nw.from_native(df_raw)
...
def test_unique_invalid_keep(nw_frame_constructor: Constructor) -> None:
    with pytest.raises(ValueError, match=r"(Got|got): cabbage"):
        nw.from_native(nw_frame_constructor(data)).unique(keep="cabbage")  # type: ignore[arg-type]


@pytest.mark.filterwarnings("ignore:.*backwards-compatibility:UserWarning")
def test_unique_none(nw_frame_constructor: Constructor) -> None:
    if "duckdb" in str(nw_frame_constructor) and DUCKDB_VERSION < (1, 3):
        pytest.skip()
    df_raw = nw_frame_constructor(data)
    df = nw.from_native(df_raw)
...
def test_unique_3069(nw_frame_constructor: Constructor) -> None:
    if "duckdb" in str(nw_frame_constructor) and DUCKDB_VERSION < (1, 3):
        pytest.skip()
