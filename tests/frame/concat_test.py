...
def test_concat_horizontal(nw_eager_constructor: ConstructorEager) -> None:
    data = {"a": [1, 3, 2], "b": [4, 4, 6], "z": [7.0, 8.0, 9.0]}
    df_left = nw.from_native(nw_eager_constructor(data), eager_only=True)

    data_right = {"c": [6, 12, -1], "d": [0, -4, 2]}
    df_right = nw.from_native(nw_eager_constructor(data_right), eager_only=True)
...
def test_concat_vertical(nw_frame_constructor: Constructor) -> None:
    data = {"a": [1, 3, 2], "b": [4, 4, 6], "z": [7.0, 8.0, 9.0]}
    df_left = (
    ...
    data_right = {"c": [6, 12, -1], "d": [0, -4, 2]}
    df_right = nw.from_native(nw_frame_constructor(data_right)).lazy()
...
def test_concat_diagonal(nw_frame_constructor: Constructor) -> None:
    data_1 = {"a": [1, 3], "b": [4, 6]}
    ...
    df_1 = nw.from_native(nw_frame_constructor(data_1)).lazy()
    df_2 = nw.from_native(nw_frame_constructor(data_2)).lazy()
...
def _from_natives(
    nw_frame_constructor: Constructor, *sources: dict[str, list[Any]]
) -> Iterator[nw.LazyFrame[Any]]:
    yield from (nw.from_native(nw_frame_constructor(data)).lazy() for data in sources)


def test_concat_diagonal_bigger(nw_frame_constructor: Constructor) -> None:
    ...
    dfs = _from_natives(nw_frame_constructor, data_1, data_2, data_3)
...
def test_concat_diagonal_invalid(
    ...
    df_1 = nw.from_native(nw_frame_constructor(data_1)).lazy()
    bad_schema = nw.from_native(nw_frame_constructor(data_2)).lazy()
