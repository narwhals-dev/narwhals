...
def test_toplevel(constructor_eager: ConstructorEager) -> None:
    if "polars" in str(constructor_eager) and POLARS_VERSION < (1,):
        pytest.skip()
    df = nw.from_native(constructor_eager({"a": [1, 2, 3], "b": [4, 5, 6]}))
    result = df.select(
        min=nw.min("a"),
        max=nw.max("a"),
        mean=nw.mean("a"),
        median=nw.median("a"),
        sum=nw.sum("a"),
        sum_h=nw.sum_horizontal("a"),
        min_h=nw.min_horizontal("a"),
        max_h=nw.max_horizontal("a"),
        mean_h=nw.mean_horizontal("a"),
        len=nw.len(),
        concat_str=nw.concat_str(nw.lit("a"), nw.lit("b")),
        any_h=nw.any_horizontal(nw.lit(True), nw.lit(True)),  # noqa: FBT003
        all_h=nw.all_horizontal(nw.lit(True), nw.lit(True)),  # noqa: FBT003
        first=nw.nth(0),
        no_first=nw.exclude("a"),
    )
    expected = {
        "min": [1, 1, 1],
        "max": [3, 3, 3],
...
def test_when_then(constructor_eager: ConstructorEager) -> None:
    ...
    result = df.select(nw.when(nw.col("a") > 1).then("b").otherwise("c"))
    expected = {"b": [6, 5, 6]}
    assert_equal_data(result, expected)
    assert isinstance(result, nw.DataFrame)
...
def test_constructors() -> None:
    ...
    if PANDAS_VERSION < (2, 2):
        pytest.skip()
    assert nw.new_series("a", [1, 2, 3], backend="pandas").to_list() == [1, 2, 3]
    arr: np.ndarray[tuple[int, int], Any] = np.array([[1, 2], [3, 4]])  # pyright: ignore[reportAssignmentType]
    result = nw.from_numpy(arr, schema=["a", "b"], backend="pandas")
    assert_equal_data(result, {"a": [1, 3], "b": [2, 4]})
    assert isinstance(result, nw.DataFrame)
    result = nw.from_numpy(
        arr, schema=nw.Schema({"a": nw.Int64(), "b": nw.Int64()}), backend="pandas"
    )
    assert_equal_data(result, {"a": [1, 3], "b": [2, 4]})
    assert isinstance(result, nw.DataFrame)
    result = nw.from_dict({"a": [1, 2, 3]}, backend="pandas")
    assert_equal_data(result, {"a": [1, 2, 3]})
    assert isinstance(result, nw.DataFrame)
    result = nw.from_arrow(pd.DataFrame({"a": [1, 2, 3]}), backend="pandas")
    assert_equal_data(result, {"a": [1, 2, 3]})
    assert isinstance(result, nw.DataFrame)


def test_join(constructor_eager: ConstructorEager) -> None:
    df = nw.from_native(constructor_eager({"a": [1, 2, 3]})).lazy()
...
def test_by_name(constructor_eager: ConstructorEager) -> None:
    df = nw.from_native(constructor_eager({"a": [1, 2, 3]})).lazy()
    result = df.select(nw.col("a").alias("b"), "a")
    expected = {"b": [1, 2, 3], "a": [1, 2, 3]}
    assert_equal_data(result, expected)
    assert isinstance(result, nw.LazyFrame)
    result_eager = df.collect().select(nw.col("a").alias("b"), "a")
    assert_equal_data(result_eager, expected)
    assert isinstance(result_eager, nw.DataFrame)
...
def test_is_duplicated_unique(constructor_eager: ConstructorEager) -> None:
    ...
    assert isinstance(df.is_duplicated(), nw.Series)
    assert isinstance(df.is_unique(), nw.Series)


def test_concat(constructor_eager: ConstructorEager) -> None:
    df = nw.from_native(constructor_eager({"a": [1, 2, 3]}), eager_only=True)
    result = nw.concat([df, df], how="vertical")
    expected = {"a": [1, 2, 3, 1, 2, 3]}
    assert_equal_data(result, expected)
    assert isinstance(result, nw.DataFrame)
    if TYPE_CHECKING:
        assert_type(result, nw.DataFrame[Any])
...
def test_tail(constructor_eager: ConstructorEager) -> None:
    df = nw.from_native(constructor_eager({"a": [1, 2, 3]}), eager_only=True).lazy()
    result = df.tail(3)
    expected = {"a": [1, 2, 3]}
    assert_equal_data(result, expected)
...
def test_hist_v1(constructor_eager: ConstructorEager) -> None:
    ...
    if "cudf" in str(constructor_eager):
        pytest.skip()
    df = nw.from_native(constructor_eager({"a": [1, 1, 2]}), eager_only=True)
...
def test_int_select_pandas() -> None:
    ...
    with pytest.raises(
        nw.exceptions.InvalidIntoExprError, match="\n\nHint:\n- if you were trying"
    ):
        nw.to_native(df.select(0))  # type: ignore[arg-type]
    with pytest.raises(
        nw.exceptions.InvalidIntoExprError, match="\n\nHint:\n- if you were trying"
    ):
        nw.to_native(df.lazy().select(0))  # type: ignore[arg-type]
...
def test_enum_v1_is_enum_unstable() -> None:
    ...
    assert issubclass(nw.Enum, nw.Enum)
    ...
    with pytest.raises(TypeError, match=r"takes 1 positional argument"):
        nw.Enum(("a", "b"))  # type: ignore[call-arg]
...
def test_cast_to_enum_v1(
    ...
    with pytest.raises(
        ...
        nw.from_native(df_native).select(nw.col("a").cast(nw.Enum))  # type: ignore[arg-type]
...
def test_v1_enum_polars() -> None:
    ...
    s = nw.from_native(pl.Series(["a", "b"], dtype=pl.Enum(["a", "b"])), series_only=True)
    assert s.dtype == nw.Enum
...
def test_v1_enum_duckdb_2550() -> None:
    ...
    result_v1 = nw.from_native(
        duckdb.sql("select 'a'::enum('a', 'b', 'c') as a")
    ).collect_schema()
    assert result_v1 == {"a": nw.Enum()}
