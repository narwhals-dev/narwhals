...
def test_map_batches_expr_compliant(nw_eager_constructor: ConstructorEager) -> None:
...
def test_map_batches_expr_numpy_array(nw_eager_constructor: ConstructorEager) -> None:
    df = nw.from_native(nw_eager_constructor(data))
    expected = df.select(
        nw.col("a")
...
def test_map_batches_expr_numpy_scalar(nw_eager_constructor: ConstructorEager) -> None:
    df = nw.from_native(nw_eager_constructor(data))
...
def test_map_batches_expr_names(nw_eager_constructor: ConstructorEager) -> None:
    df = nw.from_native(nw_eager_constructor(data))
...
def test_map_batches_exception(
    ...
    if "polars" in str(nw_eager_constructor) and POLARS_VERSION < (1, 32, 3):
