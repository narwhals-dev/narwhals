...
def test_sumh(nw_frame_constructor: Constructor) -> None:
    data = {"a": [1, 3, 2], "b": [4, 4, 6], "z": [7.0, 8.0, 9.0]}
    df = nw.from_native(nw_frame_constructor(data))
    result = df.with_columns(horizontal_sum=nw.sum_horizontal("a", nw.col("b")))
    expected = {
        "a": [1, 3, 2],
        "b": [4, 4, 6],
        "z": [7.0, 8.0, 9.0],
...
def test_sumh_nullable(nw_frame_constructor: Constructor) -> None:
...
def test_sumh_all(nw_frame_constructor: Constructor) -> None:
...
def test_sumh_aggregations(nw_frame_constructor: Constructor) -> None:
    data = {"a": [1, 2, 3], "b": [10, 20, 30]}
    df = nw.from_native(nw_frame_constructor(data))
...
def test_sumh_transformations(nw_frame_constructor: Constructor) -> None:
