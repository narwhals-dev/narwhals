...
def test_is_finite_expr(nw_frame_constructor: Constructor) -> None:
    if any(
        ...
        x in str(nw_frame_constructor)
        for x in ("pandas_constructor", "dask", "modin_constructor")
    ):
...
@pytest.mark.filterwarnings("ignore:invalid value encountered in cast")
def test_is_finite_series(nw_eager_constructor: ConstructorEager) -> None:
    if "polars" in str(nw_eager_constructor) or "pyarrow_table" in str(
        ...
        or "modin_constructor" in str(nw_eager_constructor)
    ):
        expected = {"a": [False, False, True, False]}
    else:  # pandas_nullable_constructor, pandas_pyarrow_constructor, modin_pyarrrow_constructor
        expected = {"a": [None, False, True, None]}
...
def test_is_finite_integer_column(nw_frame_constructor: Constructor) -> None:
