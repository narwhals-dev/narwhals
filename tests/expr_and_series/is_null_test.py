...
def test_null(nw_frame_constructor: Constructor) -> None:
    data_na = {"a": [None, 3, 2], "z": [7.0, None, None]}
    expected = {"a": [True, False, False], "z": [True, False, False]}
    df = nw.from_native(nw_frame_constructor(data_na))
    result = df.select(nw.col("a").is_null(), ~nw.col("z").is_null())

    assert_equal_data(result, expected)


def test_null_series(nw_eager_constructor: ConstructorEager) -> None:
    data_na = {"a": [None, 3, 2], "z": [7.0, None, None]}
    expected = {"a": [True, False, False], "z": [True, False, False]}
