...
def test_to_lowercase(constructor: Any, request: pytest.FixtureRequest) -> None:
...
def test_to_lowercase_after_alias(constructor: Any, request: pytest.FixtureRequest) -> None:
    if "pyspark" in str(constructor):
        request.applymarker(pytest.mark.xfail)
    df = nw.from_native(constructor(data))
...
def test_to_lowercase_raise_anonymous(constructor: Any, request: pytest.FixtureRequest) -> None:
    if "pyspark" in str(constructor):
        request.applymarker(pytest.mark.xfail)
