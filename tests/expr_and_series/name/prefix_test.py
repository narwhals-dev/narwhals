...
def test_prefix(constructor: Any, request: pytest.FixtureRequest) -> None:
...
def test_suffix_after_alias(constructor: Any, request: pytest.FixtureRequest) -> None:
    if "pyspark" in str(constructor):
        request.applymarker(pytest.mark.xfail)
    df = nw.from_native(constructor(data))
...
def test_prefix_raise_anonymous(constructor: Any, request: pytest.FixtureRequest) -> None:
    if "pyspark" in str(constructor):
        request.applymarker(pytest.mark.xfail)
