...
def test_selectors(nw_frame_constructor: Constructor) -> None:
...
def test_matches(nw_frame_constructor: Constructor) -> None:
    df = nw.from_native(nw_frame_constructor(data_regex))
    result = df.select(ncs.matches("[^z]a") + 1)
    expected = {"bar": [124, 457], "baz": [3.0, 6.5]}
    assert_equal_data(result, expected)


def test_numeric(nw_frame_constructor: Constructor) -> None:
    df = nw.from_native(nw_frame_constructor(data))
...
def test_boolean(nw_frame_constructor: Constructor) -> None:
    df = nw.from_native(nw_frame_constructor(data))
...
def test_string(nw_frame_constructor: Constructor) -> None:
    df = nw.from_native(nw_frame_constructor(data))
...
def test_datetime(
    ...
    if (
        ...
        or ("pyarrow" in str(nw_frame_constructor) and is_windows())
        or ("pandas" in str(nw_frame_constructor) and PANDAS_VERSION < (2,))
        or "ibis" in str(nw_frame_constructor)
        # https://github.com/pola-rs/polars/issues/23767
        # TODO(FBruzzesi): Manage polars version
        or ("polars" in str(nw_frame_constructor) and POLARS_VERSION >= (1, 32, 0))
    ):
        request.applymarker(pytest.mark.xfail)
...
def test_datetime_no_tz(nw_frame_constructor: Constructor) -> None:
    ts1 = datetime(2000, 11, 20, 18, 12, 16, 600000)
    ts2 = datetime(2020, 10, 30, 10, 20, 25, 123000)
...
def test_set_ops(
    ...
    if (
        any(x in str(nw_frame_constructor) for x in ("duckdb", "sqlframe", "ibis"))
        and not expected
    ):
...
def test_subtract_expr(nw_frame_constructor: Constructor) -> None:
    if "polars" in str(nw_frame_constructor) and POLARS_VERSION < (0, 20, 27):
...
def test_set_ops_invalid(nw_frame_constructor: Constructor) -> None:
    df = nw.from_native(nw_frame_constructor(data))
...
def test_tz_aware(
    ...
    if "polars" in str(nw_frame_constructor) and POLARS_VERSION < (1, 19):
        # bug in old polars
        pytest.skip()
    if (
