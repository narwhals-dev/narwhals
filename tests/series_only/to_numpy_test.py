...
def test_to_numpy(
    ...
    data = [1, 2, None]
    s = nw.from_native(nw_eager_constructor({"a": data}), eager_only=True)["a"].cast(
        nw.Int64
    )
...
def test_to_numpy_tz_aware(
    ...
    if (
        ("pandas_pyarrow" in str(nw_eager_constructor) and PANDAS_VERSION < (2, 2))
        or ("modin_pyarrow" in str(nw_eager_constructor) and PANDAS_VERSION < (2, 2))
        or ("pyarrow" in str(nw_eager_constructor) and is_windows())
    ):
        request.applymarker(pytest.mark.xfail)
        request.applymarker(pytest.mark.xfail)
    df = nw.from_native(
