...
# fmt: off
class BooleanFunction(Function, dtype=dtm.BOOL): ...
...
class _HorizontalBoolean(HorizontalFunction, BooleanFunction, dtype=dtm.BOOL):
    ...
    ignore_nulls: bool
class All(_BooleanUnary, Aggregation): ...
class Any(_BooleanUnary, Aggregation): ...
class AllHorizontal(_HorizontalBoolean): ...
class AnyHorizontal(_HorizontalBoolean): ...
class IsDuplicated(_BooleanUnary, LengthPreserving): ...
class IsFinite(_BooleanUnary, Elementwise): ...
class IsFirstDistinct(_BooleanUnary, LengthPreserving): ...
class IsLastDistinct(_BooleanUnary, LengthPreserving): ...
class IsNan(_BooleanUnary, Elementwise): ...
class IsNull(_BooleanUnary, Elementwise): ...
class IsNotNan(_BooleanUnary, Elementwise): ...
class IsNotNull(_BooleanUnary, Elementwise): ...
class IsUnique(_BooleanUnary, LengthPreserving): ...
class Not(_BooleanUnary, Elementwise, dispatch=renamed("not_"), dtype=map_first(lambda dtype: dtype if dtype.is_integer() else dtm.BOOL)): ...
# fmt: on
class IsBetween(TernaryFunction, BooleanFunction, Elementwise):
    __slots__ = ("closed",)
    closed: ClosedInterval


class IsInSeq(_BooleanUnary, Elementwise):
    __slots__ = ("other",)
    other: Seq[t.Any]
...
class IsInSeries(_BooleanUnary, Elementwise, Generic[NativeSeriesT_co]):
    __slots__ = ("other",)
...
class IsInExpr(BinaryFunction, BooleanFunction, Elementwise):
