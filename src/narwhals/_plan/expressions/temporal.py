...
from narwhals._plan._function import Elementwise, UnaryFunction
...
# fmt: off
class TemporalFunction(Elementwise, dispatch=DispatcherOptions(accessor_name="dt")): ...
...
class _TemporalTimeZone(_TemporalUnary, Generic[Tz]):
    __slots__ = ("time_zone",)
    time_zone: Tz
...
class _TemporalInterval(_TemporalUnary, dtype=same_dtype()):
...
class OrdinalDay(_TemporalUnary, dtype=dtm.I16): ...
...
class Date(_TemporalUnary, dtype=dtm.DATE): ...
class ToString(_TemporalUnary, dtype=dtm.STR):
    __slots__ = ("format",)
    format: str
