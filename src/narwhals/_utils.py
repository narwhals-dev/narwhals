...
class PivotColumn:
    """Information about a column generated by a pivot operation.

    Attributes:
        value: Name of the value column.
        on_value: Value from the `on` column represented by this column.
        native_name: Name assigned by the native backend.
        output_name: Narwhals name for the column, matching Polars.
    """
...
def generate_pivot_column_names(
    ...
    return [
        PivotColumn(
            value=value,
            on_value=on_value,
            native_name=(str(on_value) if single_value else f"{on_value}_{value}"),
            output_name=(
                str(on_value) if single_value else separator.join((value, str(on_value)))
            ),
        )
        for value in values
