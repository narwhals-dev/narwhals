...
top_level_functions = [
    i
    for i in nw.DataFrame(pl.DataFrame(), backend_version=(0,), level="full").__dir__()
...
top_level_functions = [
    i
    for i in nw.LazyFrame(pl.LazyFrame(), backend_version=(0,), level="full").__dir__()
    if not i[0].isupper() and i[0] != "_"
]
...
top_level_functions = [
    i
    for i in nw.Series(pl.Series(), backend_version=(1,), level="full").__dir__()
    if not i[0].isupper() and i[0] != "_"
]
