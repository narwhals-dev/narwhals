...
!!! info "Typing"
    Only the module object is part of the *typed* API: `IntoBackend` includes
    `ModuleType`, while plugin names cannot join its `Literal` unions of built-in
    backend names (and widening to `str` would stop type checkers from catching
    invalid built-in names). The string forms work at runtime; if you use them in
    type-checked code, add a `# type: ignore[arg-type]` comment.
