...
class DuckDBExprStringNamespace:
    ...
    def split(self: Self, by: str | None, *, inclusive: bool) -> DuckDBExpr:
        def func(_input: duckdb.Expression) -> duckdb.Expression:
            split_expr = FunctionExpression("str_split", _input, ConstantExpression(by))
            if inclusive:
                array_to_string_expr = FunctionExpression(
                    "array_to_string", split_expr, ConstantExpression("|")
                )
                regexp_replace_expr = FunctionExpression(
                    "regexp_replace",
                    array_to_string_expr,
                    ConstantExpression(r"\|"),
                    ConstantExpression(f"{by}|"),
                )
                return FunctionExpression(
                    "str_split", regexp_replace_expr, ConstantExpression("|")
                )
            return split_expr
