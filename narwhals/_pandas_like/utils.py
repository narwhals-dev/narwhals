...
def check_column_names_are_unique(columns: pd.Index) -> None:
    # Use this one instead of the one in `narwhals/utils.py` in order to
    # deal with non-hashable names. Seriously. scikit-learn have a test
    # with `df = pd.DataFrame([[1, 2], [3, 4]], columns=["pizza", ["a", "b"]])`.
    try:
