...
class PolarsDataFrame(PolarsFrame, CompliantDataFrame[pl.DataFrame, pl.Series]):
    ...
    @classmethod
    def from_native(cls, native: pl.DataFrame, /) -> Self:
        obj = cls.__new__(cls)
        obj._native = native
        return obj
    ...
    def clone(self) -> Self:
        return self.from_native(self.native.clone())

    def drop(self, columns: Sequence[str]) -> Self:
        return self.from_native(self.native.drop(columns))
    ...
    def from_dict(
        ...
        if not schema:
            return cls.from_native(pl.from_dict(data))
        s: Any = {
            k: (dtype_to_native(d, cls.version) if d else d) for k, d in schema.items()
        }
    ...
    def select_names(self, *column_names: str) -> Self:
        return self.from_native(self.native.select(column_names))
    ...
    def partition_by(self, by: Sequence[str], *, include_key: bool = True) -> list[Self]:
        ...
        return [self.from_native(p) for p in results]
    ...
    def unique_by(
        ...
        *,
        ...
        if not maintain_order:
            return self.from_native(self.native.sort(order_by).unique(subset, keep=keep))
    ...
    def unnest(self, columns: Sequence[str]) -> Self:
        return self.from_native(self.native.unnest(columns))
