...
class PolarsGroupBy:
    def __init__(self, df: Any, keys: list[str]) -> None:
        ...
        self.keys = keys
        self._grouped = df._native_dataframe.group_by(keys)
    ...
    def agg(self, *aggs: Any, **named_aggs: Any) -> PolarsDataFrame:
        ...
        return self._compliant_frame._from_native_dataframe(  # type: ignore[no-any-return]
            self._grouped.agg(*aggs, **named_aggs),
        )

    def __iter__(self) -> Any:
        for key, df in self._grouped:
            yield tuple(key), self._compliant_frame._from_native_dataframe(df)
