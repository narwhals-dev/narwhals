...
```py exec="1" source="above" session="null_handling"
...
=== "pandas"
    ```py exec="true" source="material-block" result="python" session="null_handling"
    import pandas as pd
...
```py exec="1" source="above" session="nan-comparisons" result="python"
import pyarrow as pa
